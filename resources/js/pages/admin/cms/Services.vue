    ...
    <div class="space-y-6">
            ...
            <table v-else class="w-full">
                ...
                <tbody class="divide-y divide-border">
                    <tr v-for="p in items" :key="p.id" class="hover:bg-muted/10">
                        ...
                        <td class="px-4 py-3"><span class="font-medium">{{ p.title }}</span><span class="text-muted-foreground text-sm block">{{ p.slug }}</span></td>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
            <div class="bg-background border border-border rounded-lg shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-semibold mb-4">{{ editingId ? 'Редактировать услугу' : 'Новая услуга' }}</h3>
...
export default {
    ...
    setup() {
        ...
        const form = ref({ site_id: '', slug: '', title: '', status: 'draft' });
        ...
        const fetchList = async () => {
            ...
            try {
                ...
                const res = await apiGet('/cms/services?' + new URLSearchParams(params).toString());
        ...
        const openCreate = () => { editingId.value = null; form.value = { site_id: filters.value.site_id || '', slug: '', title: '', status: 'draft' }; validationErrors.value = ''; showModal.value = true; };
        const editItem = (p) => { editingId.value = p.id; form.value = { site_id: String(p.site_id), slug: p.slug, title: p.title, status: p.status || 'draft' }; validationErrors.value = ''; showModal.value = true; };
        const save = async () => {
            saving.value = true;
            validationErrors.value = '';
            try {
                const payload = { site_id: Number(form.value.site_id), slug: form.value.slug.trim(), title: form.value.title.trim(), status: form.value.status };
                const res = editingId.value ? await apiPut(`/cms/services/${editingId.value}`, payload) : await apiPost('/cms/services', payload);
                const data = await res.json();
        ...
        const remove = async (p) => {
            const ok = await Swal.fire({ title: 'Удалить услугу?', text: p.title, icon: 'warning', showCancelButton: true, confirmButtonText: 'Удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!ok.isConfirmed) return;
            try {
                const res = await apiDelete(`/cms/services/${p.id}`);
