<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-semibold text-foreground">Сайты</h1>
                <p class="text-muted-foreground mt-1">Управление сайтами (мульти-сайт)</p>
            </div>
            <button
                @click="openCreate"
            ...
            <table v-else class="w-full">
                <thead class="bg-muted/30 border-b border-border">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Домен</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Город</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Корневой</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase">Действия</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    <tr v-for="s in items" :key="s.id" class="hover:bg-muted/10">
                        <td class="px-4 py-3 text-sm">{{ s.id }}</td>
                        <td class="px-4 py-3 font-medium">{{ s.domain }}</td>
                        <td class="px-4 py-3 text-sm">{{ s.city ? s.city.name : '—' }}</td>
                        <td class="px-4 py-3 text-sm">{{ s.is_primary ? 'Да' : 'Нет' }}</td>
                        <td class="px-4 py-3 text-right">
                            <button @click="editItem(s)" class="px-3 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded mr-1">Изменить</button>
                            <button @click="remove(s)" class="px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded">Удалить</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
            <div class="bg-background border border-border rounded-lg shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-semibold mb-4">{{ editingId ? 'Редактировать сайт' : 'Новый сайт' }}</h3>
                <form @submit.prevent="save" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium mb-1">Домен *</label>
                        <input v-model="form.domain" required maxlength="255" class="w-full h-10 px-3 border border-border rounded bg-background" placeholder="example.ru" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Город</label>
                        <select v-model="form.city_id" class="w-full h-10 px-3 border border-border rounded bg-background">
                            <option value="">— Без города (корневой сайт)</option>
                            <option v-for="c in cities" :key="c.id" :value="c.id">{{ c.name }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input v-model="form.is_primary" type="checkbox" class="w-4 h-4" />
                            <span class="text-sm font-medium">Корневой сайт (is_primary)</span>
                        </label>
                        <p class="text-xs text-muted-foreground mt-1">Один на проект.</p>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const form = ref({ domain: '', city_id: '', is_primary: false });
        ...
        const fetchCities = async () => {
            try {
                const res = await apiGet('/cms/cities?per_page=500');
                if (res.ok) {
                    const d = await res.json();
                    cities.value = (d.data || []).map(c => ({ ...c, region: c.region || {} }));
                }
            } catch (e) {}
        };
        ...
        const fetchList = async () => {
            ...
            try {
                ...
                const res = await apiGet(`/cms/sites?${qs}`);
        ...
        const openCreate = () => {
            ...
            form.value = { domain: '', city_id: '', is_primary: false };
        ...
        const save = async () => {
            ...
            try {
                ...
                const res = editingId.value
                    ? await apiPut(`/cms/sites/${editingId.value}`, payload)
                    : await apiPost('/cms/sites', payload);
                const data = await res.json();
        ...
        const remove = async (s) => {
            const ok = await Swal.fire({ title: 'Удалить сайт?', text: s.domain, icon: 'warning', showCancelButton: true, confirmButtonText: 'Удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!ok.isConfirmed) return;
            try {
                const res = await apiDelete(`/cms/sites/${s.id}`);
                const data = await res.json();
        ...
        return { loading, saving, error, items, cities, pagination, showModal, editingId, filters, form, debouncedFetch, setPage, openCreate, editItem, save, remove };
    },
};
</script>
