    ...
    <div class="space-y-6">
            ...
            <table v-else class="w-full">
                <thead class="bg-muted/30 border-b border-border">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Порядок</th>
                ...
                <tbody class="divide-y divide-border">
                    <tr v-for="c in items" :key="c.id" class="hover:bg-muted/10">
                        ...
                        <td class="px-4 py-3"><span class="font-medium">{{ c.title }}</span><span class="text-muted-foreground text-sm block">{{ c.slug }}</span></td>
                        <td class="px-4 py-3 text-sm">{{ c.sort_order ?? 0 }}</td>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
            <div class="bg-background border border-border rounded-lg shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-semibold mb-4">{{ editingId ? 'Редактировать категорию' : 'Новая категория' }}</h3>
                <form @submit.prevent="save" class="space-y-4">
                    ...
                    <div><label class="block text-sm font-medium mb-1">Порядок</label>
                        <input v-model.number="form.sort_order" type="number" min="0" class="w-full h-10 px-3 border border-border rounded bg-background" />
                    </div>
...
export default {
    ...
    setup() {
        ...
        const filters = ref({ q: '', site_id: '', page: 1, per_page: 15 });
        const form = ref({ site_id: '', slug: '', title: '', sort_order: 0 });
        let searchTimeout = null;
        const slugFromTitle = () => {
            if (!form.value.slug && form.value.title) form.value.slug = form.value.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/gi, '');
        };
        ...
        const fetchList = async () => {
            ...
            try {
                ...
                const res = await apiGet('/cms/product-categories?' + new URLSearchParams(params).toString());
        ...
        const openCreate = () => { editingId.value = null; form.value = { site_id: filters.value.site_id || '', slug: '', title: '', sort_order: 0 }; validationErrors.value = ''; showModal.value = true; };
        const editItem = (c) => { editingId.value = c.id; form.value = { site_id: String(c.site_id), slug: c.slug, title: c.title, sort_order: c.sort_order ?? 0 }; validationErrors.value = ''; showModal.value = true; };
        const save = async () => {
            saving.value = true;
            validationErrors.value = '';
            try {
                const payload = { site_id: Number(form.value.site_id), slug: form.value.slug.trim(), title: form.value.title.trim(), sort_order: Number(form.value.sort_order) || 0 };
                const res = editingId.value ? await apiPut(`/cms/product-categories/${editingId.value}`, payload) : await apiPost('/cms/product-categories', payload);
                const data = await res.json();
        ...
        const remove = async (c) => {
            const ok = await Swal.fire({ title: 'Удалить категорию?', text: c.title, icon: 'warning', showCancelButton: true, confirmButtonText: 'Удалить', cancelButtonText: 'Отмена', confirmButtonColor: '#dc2626' });
            if (!ok.isConfirmed) return;
            try {
                const res = await apiDelete(`/cms/product-categories/${c.id}`);
        ...
        onMounted(() => { fetchSites(); fetchList(); });
