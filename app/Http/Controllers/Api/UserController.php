...
class UserController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['message' => 'Пользователь успешно создан', 'data' => $user->load('roles')], 201);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['message' => 'Пользователь успешно обновлен', 'data' => $user->fresh('roles')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['message' => 'Ошибка при обновлении пользователя', 'error' => $e->getMessage()], 500);
        }
    }
    ...
    public function destroy(string $id): JsonResponse
    {
        $user = User::findOrFail($id);
        if ($user->id === auth()->id()) {
            return response()->json(['message' => 'Невозможно удалить самого себя'], 422);
        }
        $user->delete();
